<template>
    <view class="container">
        ...
        <view class="search-city">
            <input placeholder="输入城市名或拼音查询" class="search-input" @input="search" placeholder-class="phcolor" />
            <view class="iconfont icon-sousuo"></view>
        </view>
        <!-- 搜索为空时显示所有城市列表 -->
        <block v-if="!!!searchValue">
            <view class="citylist-content">
                <view :data-title="section.title" class="section" v-for="(section, index) in citylist" :key="section.title">
                    <view class="section-title">{{ section.title }}</view>

                    <view :class="section.style === 'inline' ? 'section-body-inline' : 'section-body'">
                        <view class="section-item" @tap="selectCity" :data-city="city" v-for="(city, index1) in section.items" :key="city.id">{{ city.city_name }}</view>
                    </view>
                </view>
            </view>
            <!-- 右侧导航栏 -->
            <view class="citylist-nav" id="test" @touchmove.stop.prevent="handleTouchmove" @touchstart="handleTouchstart" @touchend="handleTouchend">
                <view class="citylist-nav-item" @tap="navSelect" :data-index="index" v-for="(item, index) in citylist" :key="item.title">
        ...
        <block v-else>
            <view class="result-box">
                <block v-if="result.length">
                    <view :data-city="city" @tap="selectCity" class="result-item" v-for="(city, index) in result" :key="city.id">{{ city.city_name }}</view>
                </block>
                <block v-else>
    ...
    methods: {
        ...
        normalizeCityList(citys) {
            ...
            if (app.globalData.userLocation.status === 1) {
                let city = citys.find((item) => item.city_name === app.globalData.userLocation.cityName) || {
        ...
        handleTouchmove: throttle(function (e) {
            const { navTop, navItemHeight, citylist, sections } = this;
