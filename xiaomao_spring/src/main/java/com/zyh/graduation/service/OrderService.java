...
import com.zyh.graduation.entity.*;
...
public class OrderService {
    ...
    public OrderItem addOrder(Order order, String seat) {
        ...
        if(seat.indexOf("排") != -1) {
            ...
            orderItem.setSnackSeat(order.getSnackSeat());
    ...
    public List<OrderItem> getMovieOrderByCinema(Integer userId, Integer cinemaId) {
        List<Order> orderList = orderMapper.getMovieOrderByCinema(userId,cinemaId);
        List<OrderItem> orderItems = new ArrayList<>();
        for(Order order : orderList){
            OrderItem orderItem = new OrderItem();
            Movie movie = movieMapper.getMovieById(order.getItemId());
