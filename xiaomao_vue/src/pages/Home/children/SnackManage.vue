          ...
          async manageSnackInfo(){
            if (this.snackInfo.imageUrl&&this.snackInfo.firstTitle&&this.snackInfo.secondTitle&&this.snackInfo.price&&this.snackInfo.totalNumber) {
                if(this.$refs.uploadImg.files[0]){
                    ...
                    if (json.state===200){
                        if (json.data){
                            this.snackInfo.imageUrl = this.$store.state.baseURL+'/upFile/'+json.data.img;
                        }
                    }else{
                ...
                if (this.dialogTitle === '编辑小吃信息') {
                  ...
                  console.log(this.snackInfo,this.addcinemaId);
                  this.snackInfo.cinemaId = this.addcinemaId;
                  let json = await updateSnackInfo(this.snackInfo);
                  if (json.state===200){
                    this.loadCurrentPageSnack(this.currentPage,8,this.searchInput);
                    this.dialogFormVisible = false;
